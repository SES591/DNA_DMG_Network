...
def time_series_all(net, nodes_list, Nbr_Initial_States, Nbr_States, MAX_TimeStep, damage_type, damage_node):
    ...
    for initDecState in range(Nbr_Initial_States):
        ...
        for step in range(MAX_TimeStep):
            ...
            currBiState = ur.sigmoid_updating(net, prevBiState,damage_type,damage_node)
...
def net_state_transition(net, nodes_list, Nbr_States,damage_type,damage_node):
    Nbr_States = 2
    '''
    Arguments:
               1. net
...
def time_series_pa(net, nodes_list, Initial_States_List, Nbr_States, MAX_TimeStep, damage_type, damage_node):
...
def time_series_one(net, nodes_list, Initial_State, Nbr_States, MAX_TimeStep, damage_type, damage_node):
...
def edge_print(EDGE_FILE,NODE_FILE,damage_type,damage_node):
    ...
    Nbr_States = 2
    decStateTransMap = net_state_transition(net, nodes_list,Nbr_States,damage_type,damage_node)
   
    # Exports network and attractor landscape for use in cytoscape.
    nx.write_graphml(net,"network_p53_%s.graphml"%(damage_node))
    nx.write_graphml(decStateTransMap,"attractor_p53_%s.graphml"%(damage_node))
    
    attractor_file = 'attractors_p53_%s.txt'%(damage_node)
