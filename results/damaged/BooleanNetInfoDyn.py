...
def Bool_info(EDGE_FILE,NODE_FILE,maxStep,historyLength,network_filename,damage_type,damage_node):
   ...
   if 'all_initial' in timeSeries_Type:
       ...
       timeSeriesAll = tev.time_series_all(net, nodes_list, Nbr_All_Initial_States, Nbr_States, maxStep,damage_type,damage_node) # To generate time series data over all possible initial states
...
def main(argv):
   ...
   # Damage type
   damage_type = str(sys.argv[6])
   # Damage node
   damage_node = str(sys.argv[7])

   Bool_info(EDGE_FILE,NODE_FILE,maxStep,historyLength,network_filename,damage_type,damage_node)
